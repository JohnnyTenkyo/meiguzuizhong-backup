...
- [ ] 将首页“量化回测平台”更新为 `http://mglh.cc.cd/`，并将“全自动策略回测”更新为 `https://tzlh.cc.cd/cache`
- [ ] 诊断并修复 X 推文中文翻译加载失败或缓慢的问题，同时保留 Truth Social 中文内容显示
- [ ] 将猫姐（`@maojietrading`）与 Serenity（`@aleabitoreddit`）加入默认 X 信息流追踪名单
- [ ] 修复自定义 X 账号追踪的保存、查询和前端列表刷新流程
- [ ] 修复自选股与 CEO、公司 X 账号的自动关联追踪，并验证列表显示
