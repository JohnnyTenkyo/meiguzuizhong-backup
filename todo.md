...
## 腾讯云与 Cloudflare 迁移（2026-08-14）
- [x] 审查 GitHub 源码、构建方式、信息流实现与环境变量依赖（生产构建通过）
- [x] 创建独立 MariaDB 数据库、应用账户、端口和 PM2 进程
- [x] 从 tzlh7 受控环境复用已验证的股票数据源密钥，不写入 Git 仓库
- [x] 配置信息流所需的第三方服务密钥与后台任务
- [x] 将用户提供的 X 授权 Cookie 安全写入独立信息流服务，并验证人物原始动态抓取
- [x] 修复 twikit 抓取接口的用户名到用户 ID 解析，验证原始推文不再返回空数组
- [x] 替换已失效的 twikit 交易校验实现并验证 X 当前接口兼容性
- [x] 配置独立 Nginx 虚拟主机，确保不影响既有网站
- [x] 在 Cloudflare 建立 usg.cc.cd 区域、DNS、Origin CA 与 Full (strict) HTTPS
- [x] 构建并启动独立生产应用，不迁移 Manus 历史数据
- [x] 验证 usg.cc.cd 的 HTTPS、认证、股票数据与信息流核心功能
- [x] 编写站点运维记录并保存部署配置
- [x] 将本次临时 SSH 凭据保留至所有站点迁移完成后统一撤销

## X 信息流服务运行时修复（2026-08-14）
- [x] 修复 twifork 客户端与 Flask 请求之间的跨事件循环错误，恢复连续人物推文抓取
- [x] 增加回归测试并验证生产服务可连续返回人物原始动态

## 首页入口与信息流追踪修复（2026-08-14）
- [x] 将首页“量化回测平台”更新为 `http://mglh.cc.cd/`，并将“全自动策略回测”更新为 `https://tzlh.cc.cd/cache`
- [x] 诊断并修复 X 推文中文翻译加载失败或缓慢的问题，同时保留 Truth Social 中文内容显示
- [x] 将猫姐（`@maojietrading`）与 Serenity（`@aleabitoreddit`）加入默认 X 信息流追踪名单
- [x] 修复自定义 X 账号追踪的保存、查询和前端列表刷新流程
- [x] 修复自选股与 CEO、公司 X 账号的自动关联追踪，并验证列表显示
